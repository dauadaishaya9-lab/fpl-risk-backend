...
setInterval(() => {
  ...
  for (const store of [ipBuckets, userBuckets]) {
    for (const [key, value] of store) if (value.startedAt < cutoff) store.delete(key);
  }
}, WINDOW_MS).unref();
...
function validateSecurityConfig() {
  if (!DATABASE_URL) return "DATABASE_URL is required for trial enforcement.";
  if (!CLERK_ISSUER || !CLERK_JWKS_URL || CLERK_AUTHORIZED_PARTIES.length === 0) {
    return "CLERK_ISSUER, CLERK_JWKS_URL and CLERK_AUTHORIZED_PARTIES are required.";
  }
  let issuerUrl;
  let jwksUrl;
  try {
    issuerUrl = new URL(CLERK_ISSUER);
    jwksUrl = new URL(CLERK_JWKS_URL);
  } catch {
    return "CLERK_ISSUER and CLERK_JWKS_URL must be valid URLs.";
  }
  if (issuerUrl.protocol !== "https:" || jwksUrl.protocol !== "https:") {
    return "CLERK_ISSUER and CLERK_JWKS_URL must use HTTPS.";
  }
  if (jwksUrl.protocol !== "https:") return "CLERK_JWKS_URL must use HTTPS.";
  return null;
}
...
async function fetchJwks(force = false) {
  ...
  jwksRefreshPromise = (async () => {
    ...
    try {
      ...
      if (!keys.size) throw new Error("JWKS document contains no usable RS256 signing keys");
...
async function verifyClerkToken(req) {
  ...
  if (token.length > MAX_JWT_LENGTH) return { ok: false, status: 401, error: "Invalid authentication token." };
  ...
  try {
    ...
    if (header.alg !== "RS256" || header.typ !== "JWT" || typeof header.kid !== "string") throw new Error("Invalid JWT header");
    if (typeof payload.sub !== "string" || !payload.sub) throw new Error("Missing subject");
    if (typeof payload.iss !== "string" || payload.iss.replace(/\/$/, "") !== CLERK_ISSUER) throw new Error("Invalid issuer");
    if (typeof payload.azp !== "string" || !CLERK_AUTHORIZED_PARTIES.includes(payload.azp)) throw new Error("Invalid authorized party");

    const now = Math.floor(Date.now() / 1000);
    if (!Number.isFinite(payload.exp) || payload.exp <= now) throw new Error("Expired token");
    if (payload.nbf !== undefined && (!Number.isFinite(Number(payload.nbf)) || Number(payload.nbf) > now + 5)) throw new Error("Token not active");
    ...
    const valid = crypto.verify("RSA-SHA256", Buffer.from(`${parts[0]}.${parts[1]}`), publicKey, base64urlBytes(parts[2]));
    if (!valid) throw new Error("Invalid signature");

    return { ok: true, userId: payload.sub, sessionId: typeof payload.sid === "string" ? payload.sid : null, claims: payload };
  } catch (error) {
...
async function getCurrentGameweekAndDeadline() {
  ...
  const upcoming = await pool.query(`
    SELECT gameweek, deadline FROM fpl_gameweeks
  ...
  return { gameweek: Number(upcoming.rows[0].gameweek), deadline: new Date(upcoming.rows[0].deadline) };
}
...
async function consumeTrial(userId) {
  ...
  const result = await pool.query(`
    ...
    DO UPDATE SET calculation_count = user_deadline_usage.calculation_count + 1,
  ...
  if (!result.rowCount) {
    const current = await pool.query(`SELECT calculation_count FROM user_deadline_usage WHERE user_id = $1 AND gameweek = $2`, [userId, gameweek]);
  ...
  return { allowed: true, used, remaining: Math.max(0, TRIAL_LIMIT - used), gameweek, deadline };
}

async function refundTrial(userId, gameweek) {
  if (!pool) return;
  await pool.query(`
    UPDATE user_deadline_usage SET calculation_count = GREATEST(calculation_count - 1, 0), updated_at = NOW()
    WHERE user_id = $1 AND gameweek = $2
  `, [userId, gameweek]);
}

function originAllowed(origin) { return !origin || FRONTEND_ORIGINS.includes(origin); }
...
function copySafeHeaders(source, target) {
  const blocked = new Set(["connection", "keep-alive", "transfer-encoding", "upgrade", "host", "set-cookie", "access-control-allow-origin", "access-control-allow-credentials", "access-control-allow-methods", "access-control-allow-headers", "access-control-expose-headers"]);
  for (const [key, value] of source) if (!blocked.has(key.toLowerCase())) target.setHeader(key, value);
}
...
async function proxy(req, res, userId) {
  ...
  delete headers["x-authenticated-user-id"];
...
const gateway = http.createServer(async (req, res) => {
  try {
    ...
    if (bucketLimited(ipBuckets, clientIp(req), IP_LIMIT)) return json(res, 429, { error: "Too many requests. Please try again shortly." }, { "Retry-After": "60" });

    if (url.pathname === "/health" || url.pathname === "/") {
      const configError = validateSecurityConfig();
      return json(res, backendReady && !configError ? 200 : 503, {
        status: backendReady && !configError ? "ok" : "starting",
    ...
    if (!backendReady) return json(res, 503, { error: "Backend is starting. Please retry shortly." }, { "Retry-After": "3" });

    const auth = await verifyClerkToken(req);
    if (!auth.ok) return json(res, auth.status, { error: auth.error });
    if (bucketLimited(userBuckets, auth.userId, USER_LIMIT)) return json(res, 429, { error: "User request limit exceeded. Please try again shortly." }, { "Retry-After": "60" });
      ...
      try {
        ...
        if (!trial.allowed) {
          ...
          return json(res, 429, {
            ...
            code: "GAMEWEEK_TRIAL_LIMIT", used: trial.used, remaining: 0, limit: TRIAL_LIMIT,
            gameweek: trial.gameweek, resetsAt: trial.deadline.toISOString()
          }, { "Retry-After": String(retryAfter) });
        }
      } catch (error) {
        console.error("TRIAL ENFORCEMENT FAILED:", error.message);
        return json(res, 503, { error: "Usage enforcement is temporarily unavailable." });
      }
    }
    ...
    try {
      ...
      if (!res.headersSent) json(res, 502, { error: "Backend service unavailable." }); else res.end();
    }
  } catch (error) {
  ...
  return new Promise((resolve, reject) => {
    const attempt = () => {
      ...
      const finish = error => {
        ...
        if (Date.now() - startedAt >= timeoutMs) return reject(new Error(`Internal backend did not start within ${timeoutMs}ms.`));
...
async function start() {
  const configError = validateSecurityConfig();
  if (configError) {
    console.error(`SECURITY STARTUP FAILED: ${configError}`);
    process.exit(1);
  }
