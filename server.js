...
function getRankTier(rank){ return SAMPLING_BANDS.find(b=>rank>=b.min&&rank<=b.max)?.name||null; }
...
async function start(){ if(pool){try{await initDatabase();console.log("PostgreSQL connected and schema ready.");await refreshScheduler();setInterval(refreshScheduler,REFRESH_INTERVAL);}catch(error){console.error("DATABASE STARTUP FAILED:",error.message);}}else console.error("DATABASE_URL is missing. PostgreSQL persistence is disabled."); server.listen(PORT,"127.0.0.1",()=>{console.log(`Internal backend running on loopback port ${PORT}`);console.log(`Sample lock policy: ${LOCK_HOURS_BEFORE_DEADLINE} hour before deadline.`);console.log(`FPL bootstrap cache TTL: ${FPL_CACHE_TTL/1000}s.`);}); }
start();
