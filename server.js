...
async function completeGameweekAndRetireOld(gameweek){
  ...
  // Keep only the newest completed mega-dataset's sampled managers.
...
async function getCompletedRiskData(){ const r=await pool.query(`SELECT gameweek,season,total_managers,created_at FROM fpl_gameweeks WHERE status='complete' ORDER BY gameweek DESC LIMIT 1`); if(!r.rowCount)return null; const gw=r.rows[0]; const bands=getSamplingBands(gw.total_managers); const rows=await pool.query(`SELECT gameweek,manager_id,locked_rank,locked_tier,manager_name,team_name,overall_points_at_lock,picks,active_chip,captain,triple_captain FROM fpl_sample_managers WHERE gameweek=$1 AND picks IS NOT NULL ORDER BY locked_rank ASC`,[gw.gameweek]); return {season:gw.season,gameweek:gw.gameweek,totalManagers:gw.total_managers,bands:bands.map(b=>buildBandFromRows(b,rows.rows.filter(r=>r.locked_tier===b.name))),createdAt:gw.created_at,samplingPolicy:{lockHoursBeforeDeadline:LOCK_HOURS_BEFORE_DEADLINE,rankSource:"overall standings at lock time",picksSource:"manager GW picks after deadline",sampling:"deterministic random rank positions; 60 managers per million-rank band from 1,000,001 onward; final band ends at current FPL total managers"}}; }
