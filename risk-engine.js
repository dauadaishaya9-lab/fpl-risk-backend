...
function buildExposure(rows) {
  const ownership = new Map(), captaincy = new Map(), tripleCaptaincy = new Map();
  for (const row of rows) {
    for (const pick of row.picks || []) { const id = Number(pick.element); ownership.set(id, (ownership.get(id) || 0) + 1); }
    const captain = (row.picks || []).find(pick => pick.is_captain === true);
    if (captain) { const id = Number(captain.element); captaincy.set(id, (captaincy.get(id) || 0) + 1); if (Number(captain.multiplier) === 3) tripleCaptaincy.set(id, (tripleCaptaincy.get(id) || 0) + 1); }
  }
