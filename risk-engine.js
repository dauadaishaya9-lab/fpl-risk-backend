...
export async function getManagerContext(fplId) {
  ...
  if (!Number.isSafeInteger(rank) || rank < 1) throw new Error("FPL manager rank is unavailable");
  const tier = tierForRank(rank);
  if (!tier) throw new Error("Manager rank is outside configured tiers");
  return {
    managerId: fplId,
...
async function latestCompletedGameweek() {
  ...
  const result = await pool.query(`
    ...
    FROM fpl_gameweeks WHERE status = 'complete'
...
export async function getUsage(userId) {
  if (!pool) throw new Error("DATABASE_URL is required");
  const result = await pool.query(`
    SELECT gameweek, deadline_at, calculation_count
    FROM user_deadline_usage
    WHERE user_id = $1
    ORDER BY gameweek DESC LIMIT 1
  `, [userId]);
  const row = result.rows[0];
  return { used: Number(row?.calculation_count || 0), remaining: Math.max(0, TRIAL_LIMIT - Number(row?.calculation_count || 0)), limit: TRIAL_LIMIT, gameweek: row ? Number(row.gameweek) : null, resetsAt: row?.deadline_at || null };
}
