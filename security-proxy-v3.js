...
import pg from "pg";
import { analyzeRiskForUser, getTopFiveForUser, getUsage, verifyFplEntry, pool, TRIAL_LIMIT } from "./risk-engine.js";
...
function cors(origin) { return origin && FRONTEND_ORIGINS.includes(origin) ? { "Access-Control-Allow-Origin": origin, "Access-Control-Allow-Credentials": "true", "Access-Control-Allow-Methods": "GET,POST,OPTIONS", "Access-Control-Allow-Headers": "Authorization,Content-Type", Vary: "Origin" } : {}; }
...
function limited(store, key, limit) { const now = Date.now(), current = store.get(key); if (!current || now - current.startedAt >= WINDOW_MS) { if (store.size >= MAX_BUCKETS) store.delete(store.keys().next().value); store.set(key, { startedAt: now, count: 1 }); return false; } current.count += 1; return current.count > limit; }
...
function tokenFrom(req) { const auth = req.headers.authorization; if (typeof auth === "string") { const m = auth.match(/^Bearer\s+(.+)$/i); if (m) return m[1].trim(); } const m = String(req.headers.cookie || "").match(/(?:^|;\s*)__session=([^;]+)/); if (!m) return null; try { return decodeURIComponent(m[1]); } catch { return null; } }
function configError() { if (!DATABASE_URL) return "DATABASE_URL is required."; if (!CLERK_ISSUER || !CLERK_JWKS_URL || !CLERK_AUTHORIZED_PARTIES.length) return "Clerk authentication is not configured."; try { if (new URL(CLERK_ISSUER).protocol !== "https:" || new URL(CLERK_JWKS_URL).protocol !== "https:") return "Clerk URLs must use HTTPS."; } catch { return "Invalid Clerk URL configuration."; } return null; }
...
const gateway = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/health" || url.pathname === "/") return json(res, backendReady && !configError() ? 200 : 503, { status: backendReady && !configError() ? "ok" : "starting", securityGateway: true, backendReady, authentication: !configError(), fplIdentityStorage: false });
    ...
    // Stateless FPL identity: the Team/Entry ID is verified live and never stored.
    if (url.pathname === "/api/account/fpl") {
      if (req.method === "GET") return json(res, 200, { linked: false, requiresTeamId: true, stored: false }, cors(origin));
      if (req.method !== "POST") return json(res, 405, { error: "Method not allowed" }, { Allow: "GET,POST,OPTIONS" });
      try {
        const body = await readBody(req), verified = await verifyFplEntry(Number(body.fplId));
        return json(res, 200, { verified: true, stored: false, ...verified }, cors(origin));
      } catch (error) { return json(res, error.status || 400, { error: error.message, code: error.code || "FPL_TEAM_VERIFICATION_FAILED" }, cors(origin)); }
    }
    ...
    if (url.pathname === "/api/calculator/templates") {
      ...
      if (!Number.isSafeInteger(fplId) || fplId <= 0) return json(res, 409, { error: "Enter your current-season FPL Team ID.", code: "FPL_TEAM_ID_REQUIRED" }, cors(origin));
      try { return json(res, 200, await getTopFiveForUser(auth.userId, fplId), cors(origin)); }
    ...
    if (url.pathname === "/api/calculator/analyze") {
      ...
      let trial;
      try { trial = await consumeTrial(auth.userId); } catch { return json(res, 503, { error: "Usage enforcement is temporarily unavailable." }); }
      ...
      try {
        const body = await readBody(req), fplId = Number(body.fplId);
        if (!Number.isSafeInteger(fplId) || fplId <= 0) throw Object.assign(new Error("Enter your current-season FPL Team ID."), { status: 409, code: "FPL_TEAM_ID_REQUIRED" });
        const result = await analyzeRiskForUser(auth.userId, fplId, { playerId: Number(body.playerId), owns: body.owns, captain: body.captain, tripleCaptain: body.tripleCaptain, expectedPoints: Number(body.expectedPoints) });
        return json(res, 200, { ...result, usage: await getUsage(auth.userId) }, cors(origin));
      } catch (error) { await refundTrial(auth.userId, trial.gameweek); return json(res, error.status || 400, { error: error.message, code: error.code || "CALCULATOR_ERROR" }, cors(origin)); }
    }

    return json(res, 404, { error: "Not found" }, cors(origin));
  } catch (error) { console.error("GATEWAY ERROR:", error.message); if (!res.headersSent) json(res, 500, { error: "Internal server error." }); else res.end(); }
});
...
async function init() {
  ...
  // Remove the obsolete persistent FPL identity tables created by earlier versions.
  await usagePool.query(`DROP TABLE IF EXISTS fpl_current_account_links`);
  await usagePool.query(`DROP TABLE IF EXISTS fpl_account_links`);
  await usagePool.query(`CREATE TABLE IF NOT EXISTS user_deadline_usage (user_id TEXT NOT NULL, gameweek INTEGER NOT NULL, deadline_at TIMESTAMPTZ NOT NULL, calculation_count INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), PRIMARY KEY (user_id, gameweek), CONSTRAINT user_deadline_usage_count_nonnegative CHECK (calculation_count >= 0))`);
  await usagePool.query(`CREATE INDEX IF NOT EXISTS idx_user_deadline_usage_deadline ON user_deadline_usage(deadline_at)`);
  process.env.PORT = String(INTERNAL_PORT);
  await import("./server.js");
  backendReady = true;
  gateway.listen(PUBLIC_PORT, "0.0.0.0", () => console.log(`Stateless FPL security gateway listening on ${PUBLIC_PORT}; FPL Team IDs are not persisted.`));
}
init().catch(error => { console.error("SECURITY GATEWAY FAILED:", error.message); process.exit(1); });
